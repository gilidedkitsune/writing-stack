...
**Before running anything, check connection status in `../SKILL.md` ("What's connected").** Google (GA4 + Search Console) is connected for bolt.new through this skill. DataForSEO, Firecrawl, and PageSpeed/CrUX are not. **AI-answer visibility is connected via `bolt-cairrot`** (live REST puller). Profound was retired Sep 8 2026: do not read it, and never estimate these numbers. If a trigger below calls a skill whose backend isn't connected, skip it (say so in one line) or ask the user for the data — never fabricate volumes, difficulty, or rankings. Also skip any trigger when the user already handed you the data it would fetch (e.g. they gave you keywords → skip keyword research).
...
| The user mentions a competitor by name or asks to differentiate | Run `/competitor-analysis` on the named competitor. *(Data-heavy parts need DataForSEO; the strategic read does not.)* For AI share of voice against the named competitor, run a `bolt-cairrot` competitors pull (live). |
...
| GEO priority is high, the user asks where bolt.new shows up in AI answers, or an idea's "GEO opportunity" needs grounding | Pull Cairrot (the MCP connector's `search_mentions` and `search_citations`, or the `bolt-cairrot` puller) for visibility, named-vs-cited, per-topic and per-model breakdowns, and the competitor leaderboard, and state the window. *(Profound retired Sep 8 2026. If Cairrot returns nothing, say so. Never guess.)* |
...
| GEO priority is high (user stated it, or topic targets question/definition/comparison queries) — and by default for every blog or website piece bound for bolt.new | Run `/aeo-craft` (Sculpt mode) after the stops-slop audit: the house AEO pass — term winnability, question tree, answer-first blocks, definitional blocks, receipts, liftable formats, FAQ, freshness, and the claim-safety glaze. Replaces the generic `/geo-content-optimizer` in this stack. For definitional/comparison pieces, also run `/aeo-craft` Plan mode BEFORE the outline is locked. |
...
| Skill | When to offer | What it does |
|-------|--------------|-------------|
| `/performance-reporter` | User wants a report/dashboard on content performance | SEO/GEO report with KPI tracking — **feed it real numbers from this skill's `ga4`/`gsc` pulls; for AI-answer KPIs pull `bolt-cairrot` live and label the window** |
...
| **Cairrot** (MCP connector, or `bolt-cairrot` puller) | AI answer visibility for bolt.new (ChatGPT, Gemini, Claude, Perplexity, AI Overviews, DeepSeek, Grok) | ✅ connected: MCP per user (setup in `aeo-craft`), REST puller on Taylor's machine | Visibility, named vs cited, per-topic and per-model, competitor leaderboard, sentiment, crawler hits. Profound retired Sep 8 2026. |
...
RESEARCH PHASE
  ...
  └─ Need AI-answer visibility (GEO)? → bolt-cairrot  (live REST puller)
  └─ Do we have keywords?
...
DRAFT COMPLETE
  └─ Will this live on bolt.new?
       ...
       │    ├─ AEO pass (default for bolt.new pieces) → /aeo-craft
       │    └─ Has FAQ/HowTo? → /schema-markup-generator
       └─ No (social, email, internal) → skip SEO/GEO skills

POST-PUBLISH
  └─ Tracking performance? → bolt-seo-geo ga4/gsc (live) + bolt-cairrot (live) → /performance-reporter, /content-refresher
